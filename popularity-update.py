...
def process_one(m: dict, idx: int, total: int, update_file_path: str):
    ...
    prev = m.get('popularity')
    prev_is_num = isinstance(prev, (int, float))
    if not FORCE_REFRESH and prev_is_num:
        try:
            if abs(float(prev) - float(popularity)) < 1e-9:
                print("  -> Без изменений.")
                return 'unchanged'
        except Exception:
            pass
    ...
    if prev_is_num:
        print(f"  -> Изменено: {prev!r} -> {popularity!r} (записано в NDJSON).")
        return 'changed'
    else:
        print(f"  -> Добавлено: popularity={popularity!r} (записано в NDJSON).")
        return 'added'
...
def update_popularity(movies_data_path="movies-data.json", update_file_path="popularity-update.ndjson"):
    ...
    print(f"\nГотово. Изменилось: {changed}. Добавлено новых: {added}. Без изменений: {unchanged}. Записано обновлений: {changed + added}. Затем примените обновления к основной базе (merge).")
